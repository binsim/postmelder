...
//measureMean
void measureMean(int cycles) {  //liest die Sensoren "cycles" mal aus und bildet den Mittelwert
...
bool belegt() {
  ...
  if (meanDistances[0] <= threshhold1 && meanDistances[1] <= threshhold2) {  //wenn beide unter Threshhold, dann auslösen
    Serial.println("BELEGT");
    Serial.println("----------------");
    return true;
  } else {
...
//geleert
bool geleert() {  //stellt fest, ob das Postfach 1min lang leer war (Überprüfung ca. alle 20s -> 3x)
  ...
  if (millis() >= (zeit + cycle * 20000)) {  //ca. alle 20s
    Serial.println("MESSEN");

    if (belegt()) {  //wenn belegt, 1 hochzählen
      counter++;
    }
    cycle++;
  }
  ...
  if (counter == 0 && cycle == 3) {  //wenn fertig durchgelaufen und nicht belegt
    cycle = 0;                       //von neuem anfangen
    return true;
  } else if (cycle == 3) {  //wenn fertig durchgelaufen und belegt
    cycle = 0;              //von neuem anfangen
    counter = 0;
    return false;
  } else {
...
// loop
void loop() {
  if (!ausgeloest) {  //wenn ausgeloest, dann grüne LED an und publishen
    if (belegt()) {
      ausgeloest = true;
  ...
  } else if (geleert()) {  //wenn wieder geleert, dann grüne LED aus und publishen
